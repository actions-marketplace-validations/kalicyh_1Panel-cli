...
fn v1_base(cfg: &OnePanelConfig) -> String {
    format!(
        "{}://{}:{}/api/v1",
        cfg.scheme,
        clean_host(&cfg.host),
...
pub async fn list_composes(cfg: &OnePanelConfig, info: Option<&str>) -> Result<Vec<ComposeInfo>> {
    match list_composes_v2(cfg, info).await {
        Ok(v) => Ok(v),
        Err(_) => list_composes_v1(cfg, info).await,
    }
}

async fn list_composes_v2(cfg: &OnePanelConfig, info: Option<&str>) -> Result<Vec<ComposeInfo>> {
...
async fn list_composes_v1(cfg: &OnePanelConfig, info: Option<&str>) -> Result<Vec<ComposeInfo>> {
    let client = get_client(cfg);
    let (token, ts) = auth_headers(&cfg.api_key);

    let resp = client
        .post(format!("{}/containers/compose/search", v1_base(cfg)))
        .header("1Panel-Token", token)
        .header("1Panel-Timestamp", ts)
        .json(&serde_json::json!({
            "page": 1,
...
pub async fn test_connection(cfg: &OnePanelConfig) -> Result<()> {
    match test_connection_v2(cfg).await {
        Ok(()) => Ok(()),
        Err(_) => test_connection_v1(cfg).await,
    }
}

async fn test_connection_v2(cfg: &OnePanelConfig) -> Result<()> {
    let client = get_client(cfg);
    let (token, ts) = auth_headers(&cfg.api_key);
...
async fn test_connection_v1(cfg: &OnePanelConfig) -> Result<()> {
    let client = get_client(cfg);
    let (token, ts) = auth_headers(&cfg.api_key);
    let url = format!("{}/websites/list", v1_base(cfg));
...
pub async fn load_image(cfg: &OnePanelConfig, remote_path: &str) -> Result<()> {
    match load_image_v2(cfg, remote_path).await {
        Ok(()) => Ok(()),
        Err(_) => load_image_v1(cfg, remote_path).await,
    }
}

async fn load_image_v2(cfg: &OnePanelConfig, remote_path: &str) -> Result<()> {
    let client = get_client(cfg);
    let (token, ts) = auth_headers(&cfg.api_key);
...
async fn load_image_v1(cfg: &OnePanelConfig, remote_path: &str) -> Result<()> {
    let client = get_client(cfg);
    let (token, ts) = auth_headers(&cfg.api_key);
    let url = format!("{}/containers/image/load", v1_base(cfg));
...
pub async fn read_file(cfg: &OnePanelConfig, path: &str) -> Result<String> {
    match read_file_v2(cfg, path).await {
        Ok(v) => Ok(v),
        Err(_) => read_file_v1(cfg, path).await,
    }
}

async fn read_file_v2(cfg: &OnePanelConfig, path: &str) -> Result<String> {
    let client = get_client(cfg);
    let (token, ts) = auth_headers(&cfg.api_key);
...
async fn read_file_v1(cfg: &OnePanelConfig, path: &str) -> Result<String> {
    let client = get_client(cfg);
    let (token, ts) = auth_headers(&cfg.api_key);
    let url = format!("{}/files/content", v1_base(cfg));
...
async fn update_compose_v2(cfg: &OnePanelConfig, path: &str, content: &str) -> Result<()> {
    let client = get_client(cfg);
    let (token, ts) = auth_headers(&cfg.api_key);
